<!-- 年/月/日/时/分：2022/6/16/11/58 -->
<!-- IDE名称：WebStorm -->
<!-- 开发系统：Windows11 -->
<!-- 作者：JinPeng -->

<template>
  <div v-if="userInfo" class="h-full bg-zinc-200 dark:bg-zinc-800 duration-400 overflow-auto xl:pt-1">
    <div
      ...
      <div class="h-full w-full px-1 pb-4 text-sm mt-2 xl:w-2/3 xl:pb-0">
        ...
        <div class="py-1 xl:absolute xl:right-[16%] xl:text-center" @click="onAvatarClick">
          ...
          <div class="relative w-[80px] h-[80px] group xl:cursor-pointer xl:left-[50%] xl:translate-x-[-50%]">
            ...
            <div class="absolute top-0 rounded-[50%] w-full h-full bg-[rgba(0,0,0,.4)] hidden xl:group-hover:block">
              <m-svg-icon name="change-header-image" class="w-2 h-2 m-auto mt-2"/>
        ...
        <div class="py-1 xl:flex xl:my-1">
          ...
          <m-input v-model="userInfo.introduction" class="w-full" type="textarea"
                   max="50"></m-input>
        </div>
        <!-- 保存修改 -->
        <m-button
...
const inputFileTarget = ref(null)// 隐藏域
...
const onLogoutClick = () => {
  ...
  confirm('您确定要退出登录吗?').then(() => store.dispatch("userInfo/logout"))
}
...
watch(isDialogVisible, (val) => {
  if (!val) inputFileTarget.value.value = null
})
